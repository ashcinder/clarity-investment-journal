...
日常录入只需 **资产名称、当前金额**，收益部分可以选择填写 **收益率** 或 **收益额**。当前金额填写平台显示的资产总价值（已包含盈亏），网格填写当前总权益。资产类型默认跟随账户，用于配置比例图；无需填写代码、币数 / 份额、成本单价、当前价格或杠杆。币种由账户决定，直接显示在金额标签中。
...
在“账户与资产 → 进入账户”添加、编辑或删除资产。资产列表只显示名称、投入、收益金额、收益率和当前价值。已有记录中的当前价值与收益率自动带入简化表单，历史数量、价格及流水继续保留；保存会新增日期化本金更正与估值，避免把全部本金重复记成一次投入。存在账户未分配资金时，可以选择用它作为本金来源。
...
领域和独立后端测试共 52 项，覆盖持仓数量、市值、网格权益、多资产分类、收益、汇率、交易日历、自动定投、跨日、持久化重启、版本冲突与两种清空模式。分离架构测试使用临时数据库、后端端口 44318 和前端端口 44319，不接触个人账本；测试前先执行 `npm run build`。它会验证前端页面、API 代理读写、后端重启和清空。
